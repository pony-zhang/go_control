...
- 🔄 **新架构数据流**: 从外部请求到物理执行的完整流程
- 📋 **主要数据结构**: Task、MotionCommand、IPCMessage、HAL接口
...
### 🎯 新手入门
1. 阅读 [architecture.md](./architecture.md) 了解四层主架构和HAL设计
2. 查看 [development.md](./development.md) 的"分层开发环境"部分
3. 运行 `make build` 构建系统
4. 使用 `./bin/control` 启动控制系统
5. 查看 `config.yaml` 了解HAL和水平层配置

### 🔧 功能开发
1. 参考 [development.md](./development.md) 的"分层架构概念"
2. 根据开发需求选择对应层次：
   - **新设备协议**: HAL协议接口实现
   - **新抽象命令**: 业务逻辑层开发
   - **新任务类型**: 任务编排层开发
...
### 🎯 学习路径
1. **基础概念**: 阅读 [architecture.md](./architecture.md)
2. **开发入门**: 查看 [development.md](./development.md) 的分层开发部分
3. **数据流理解**: 学习 [data-flow.md](./data-flow.md)
4. **HAL开发**: 重点学习HAL协议接口和资源管理
5. **水平层开发**: 掌握业务逻辑、任务编排、服务协调层开发
6. **高级主题**: 分层测试、性能优化、部署运维

---

*本文档随系统持续更新，最后更新时间: 2025-09-27*
*当前版本: v2.0.0 - 支持HAL和水平层架构*